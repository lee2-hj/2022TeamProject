function change(){
    ...
    if(overlap.test(password)){
        document.getElementById('pass').innerHTML='* 같은 문자 3개 이상은 입력이 불가능합니다.';
        check =false;
    }else if(password.search(space) !=-1){
        document.getElementById('pass').innerHTML='*공백은 입력이 불가능합니다.';
        check =false;
    }
    if(password === ""){
