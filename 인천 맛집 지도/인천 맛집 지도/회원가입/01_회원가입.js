function check(){
    ...
    if(name === ""){
        ...
        document.getElementById('nameError').innerHTML='*공백은 입력이 불가능합니다.';
        check = false;
    }else{       
    ...
    //비밀번호 확인
    if(!regPass.test(password) || password.length<8){ document.getElementById('passError').innerHTML='*영문,숫자,특수문자 조합 8~25자리 아상 입력가능합니다.'; check = false;}
    if((/([a-zA-Z0-9])\1{2,}/g.test(password))){document.getElementById('passError').innerHTML='*3번이상 연속된 숫자는 입력이 불가능합니다.'; check = false;}
    if(password.search(/\s/) != -1){
        document.getElementById('passError').innerHTML='*공백은 입력이 불가능합니다.';
        check =false;
    }
    if(password ===""){
    ...
    if(check == true){
        ...
        setTimeout(function(){
            alert('회원가입이 완료되었습니다.');
            location.replace('J:/이희장/연습/팀 프로젝트/인천 맛집 지도/인천 맛집 지도/홈화면/01_홈화면.html');
        },0);
    }
